...
def get_tm(primer, oligo_conc, na_conc, mg_conc):
    ...
    ----------
    primer : str
        A string of bases, including degeneracies.
    oligo_conc : float
        Olgionucleotide concentration (μM) in PCR reaction.
    na_conc : float
        Na+ concentration (mM) in PCR solution.
    mg_conc : float
        Mg++ concentration (mM) in PCR solution.

    Returns
    -------
    out : list of floats
        List of minimum, maximum, and average melting temperatures
...
def _get_uncorrected_tm(enthalpy, entropy, oligo_conc):
    ...
    Parameters
    ----------
    enthalpy, entropy : float
        Enthalpy and Entropy estimations
    oligo_conc : float
        Olgionucleotide concentration (μM) in PCR reaction.
    ...
    -------
    float
        Uncorrected melting temperature in Kelvin.
...
def _monovalent_correction(primer, na_conc, uncorrected_tm):
    ...
    na_conc : float
        Na+ concentration (mM) in PCR solution.
    uncorrected_tm : float
        Temperature calculated using nearest-neighbor parameters described
        in Santalucia, 1998[2]_.

    Returns
    -------
    float
        Corrected melting temperature in degrees Celcius
...
def _divalent_correction(primer, na_conc, mg_conc, uncorrected_tm, ratio):
    ...
    uncorrected_tm : float
        ...
        in Santalucia, 1998[2]_.
    ratio : float
        Special ratio of Mg++ ions to Na+ ions.
...
def _min_primer_conversion(primer):
    ...
    -------
    primer : str
        A string of bases, with degeneracies removed
...
def _max_primer_conversion(primer):
    ...
    See Also
    --------
    _min_primer_conversion : Convert primer to sequence with least energy
